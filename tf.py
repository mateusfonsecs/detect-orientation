...
def find_anlgle(image):
    ...
    magnitude_spectrum = cv2.resize(magnitude_spectrum, (720,640))
    ...
    for line in linesP:
        ...
        if angle > 45:
            ...
            angle += 90
        if abs(angle) != 90 and abs(angle) != 0:
            angles.append(angle)
...
if __name__ == '__main__':
    ...
    angle, magnitude_spectrum, binary_spectrum, white_image = find_anlgle(image)
    print(angle)
    cv2.imshow('Espectro de Frequencia', magnitude_spectrum)
    cv2.imshow('Espectro de Frequencia Binarizado', binary_spectrum)
    cv2.imshow('Espectro', white_image)
    # Aguarde uma tecla pressionada e feche as janelas
    cv2.waitKey(0)
